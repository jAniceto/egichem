...
PIPELINE_32_PATH = 'calculators/static/calculators/ml-models/ml-gb-nonpolar-model-32.mod'
PIPELINE_64_PATH = 'calculators/static/calculators/ml-models/ml-gb-nonpolar-model-64.mod'
MODEL_VARIABLES = ['T', 'viscosity', 'solute.M2', 'solute.Pc', 'solvent.M1']
...
def main():
    ...
    parser = argparse.ArgumentParser(prog='ml-nonpolar', description='Nonpolar systems diffusivity estimator')
    ...
    parser.add_argument('-p',
                        ...
                        type=float, 
                        nargs=5, 
                        metavar=('TEMPERATURE', 'VISCOSITY', 'SOLUTE_MOLECULAR_MASS', 'SOLUTE_CRITICAL_PRESSURE', 'SOLVENT_MOLECULAR_MASS'),
                        help='Temperature (K), Viscosity (cp), Solute molecular mass (g/mol), Solute critical pressure (bar), solvent molecular mass (g/mol). Provided in this order.')
    ...
    if args.properties:
        if len(args.properties) != 5:
            parser.error('You must provide the 5 properties in the correct order: temperature, viscosity, solutemolecularmass, solutecriticalpressure, and solventmolecularmass.')
    ...
    elif (args.temperature or args.viscosity or args.solutemolecularmass or args.solutecriticalpressure or args.solventmolecularmass):
        if not (args.temperature and args.viscosity and args.solutemolecularmass and args.solutecriticalpressure and args.solventmolecularmass):
            parser.error('All parameters must be provided when specifying one of --temperature, --viscosity, --solutemolecularmass, --solutecriticalpressure, or --solventmolecularmass.')
...
def calc_D12(temperature, viscosity, solutemolarmass, solutecriticalpressure, solventmolarmass):
    # Create a single point Dataframe
    data = {
        'T': [temperature],
        'viscosity': [viscosity],
